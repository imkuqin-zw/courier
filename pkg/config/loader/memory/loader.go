...
// Snapshot returns a snapshot of the current loaded conf
...
func (m *memory) Get(key string) (reader.Value, error) {
	if !m.loaded() {
		if err := m.Sync(); err != nil {
			return nil, err
		}
...
func (m *memory) Watch(prefix string) (loader.Watcher, error) {
	value, err := m.Get(prefix)
	if err != nil {
		return nil, err
	}
